...
(function(){
  ...
  /* ── Address autocomplete (Photon/Komoot, AU only) ── */
  function fmtAddr(p){
    /* Build a clean "street, suburb STATE postcode" string from Photon properties */
    var parts=[];
    var street='';
    if(p.housenumber&&p.street) street=p.housenumber+' '+p.street;
    else if(p.street) street=p.street;
    else if(p.name&&p.street) street=p.name;
    if(street) parts.push(street);
    var suburb=p.city||p.town||p.village||p.suburb||p.locality||'';
    if(suburb) parts.push(suburb);
    var state=p.state||'';
    var pc=p.postcode||'';
    var tail='';
    if(state&&pc) tail=state.replace('Australian Capital Territory','ACT').replace('New South Wales','NSW').replace('Northern Territory','NT').replace('Queensland','QLD').replace('South Australia','SA').replace('Tasmania','TAS').replace('Victoria','VIC').replace('Western Australia','WA')+' '+pc;
    else if(state) tail=state;
  ...
  function addrAuto(inputEl,dropEl){
    ...
    function pick(val){inputEl.value=val;close();inputEl.dispatchEvent(new Event('input',{bubbles:true}));}
    function render(items){
      dropEl.innerHTML=''; results=items; active=-1;
    ...
    function setActive(n){
      var lis=dropEl.querySelectorAll('li:not(.loading)');
    ...
    inputEl.addEventListener('input',function(){
      ...
      timer=setTimeout(function(){
        fetch('https://photon.komoot.io/api/?q='+encodeURIComponent(q)+'&countrycode=au&limit=6&lang=en')
          ...
          .then(function(data){
            ...
            (data.features||[]).forEach(function(f){
              var p=f.properties||{};
              /* Only include results with a street — skip parks, POIs etc */
              if(!p.street&&!p.name) return;
              var addr=fmtAddr(p);
              if(addr&&!seen[addr]){seen[addr]=1;items.push(addr);}
            });
            render(items);
          })
    ...
    document.addEventListener('click',function(e){
      if(!inputEl.contains(e.target)&&!dropEl.contains(e.target)) close();
    });
  }
