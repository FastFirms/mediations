...
(function(){
  ...
  var h=document.querySelector('.phero[style]');
  if(!h)return;
  var urls=(h.style.backgroundImage||'').match(/url\([^)]+\)/g);
  if(!urls||!urls.length)return;
  var img=urls[urls.length-1];
  h.style.backgroundImage=img;
  h.style.backgroundSize='100% auto';
  h.style.backgroundPosition='center bottom';
  h.style.backgroundRepeat='no-repeat';
  h.style.backgroundColor='#f7faf7';
  h.style.paddingBottom=Math.round(window.innerWidth*0.5625)+'px';
})();
</script>
</body>
</html>"""
...
def quiz_card():
    ...
    return """<div id="hero-check" style="background:#eaf3ea;color:var(--ink);border-radius:22px;padding:clamp(24px,3vw,34px);box-shadow:0 2px 18px rgba(30,96,64,.10);border:1px solid rgba(30,96,64,.12)">
  <div id="qmintro" style="margin-bottom:18px">
    <p style="font-family:var(--body-serif);font-size:clamp(1.1rem,2vw,1.4rem);font-weight:400;color:var(--ink);margin:0 0 8px;line-height:1.3">Will Mediation Work for Me?</p>
    <p style="font-size:.84rem;color:rgba(30,60,40,.55);margin:0 0 16px;line-height:1.55">Around 2 minutes &mdash; tailored to your situation.</p>
    <button id="qstartbtn" onclick="qStart()" style="display:flex;align-items:center;gap:8px;background:var(--sage);border:1px solid var(--sage);color:#fff;padding:12px 20px;border-radius:11px;font-size:.92rem;font-family:var(--sans);cursor:pointer;width:100%;justify-content:center;transition:background .16s,border-color .16s">
      Start the assessment <span style="font-size:1.1em">&#8594;</span>
    </button>
    <p style="font-size:.74rem;color:rgba(30,60,40,.42);margin:14px 0 0;line-height:1.5">This tool gives general guidance only and is not legal advice. If safety is a concern, please call 000 or 1800RESPECT (1800 737 732).</p>
  </div>
  ...
  <div id="quiz60" style="display:none">
    ...
    <div class="qstep" id="qresult" hidden>
      <div id="qbadge"></div>
      <p id="qverdict" style="font-family:var(--body-serif);font-size:1.15rem;line-height:1.35;margin:0 0 10px;color:var(--ink)"></p>
      <p id="qnote" style="font-size:.86rem;line-height:1.65;color:rgba(30,60,40,.72);margin:0 0 16px"></p>
      ...
      <p style="font-size:.72rem;color:rgba(30,60,40,.42);margin:12px 0 0;line-height:1.5;text-align:center">General guidance only &mdash; not legal advice. If safety is a concern, call 1800RESPECT (1800 737 732).</p>
    </div>
  </div>
</div>
<style>
.qprogress{height:3px;background:rgba(30,96,64,.15);border-radius:999px;margin-bottom:22px;overflow:hidden}
...
#quiz60 .qlabel{display:flex;align-items:center;gap:8px;font-size:.72rem;letter-spacing:.06em;text-transform:uppercase;color:rgba(30,60,40,.45);margin:0 0 14px;font-family:var(--sans);line-height:1.4;flex-wrap:wrap}
#quiz60 .qopts{display:grid;gap:7px}
#quiz60 .qbtn{display:flex;align-items:center;gap:12px;text-align:left;border:1px solid rgba(30,96,64,.18);background:#fff;padding:11px 15px;border-radius:11px;font-size:.88rem;color:var(--ink);cursor:pointer;transition:background .16s,border-color .16s,transform .16s,color .16s;font-family:var(--sans);line-height:1.4;width:100%}
#quiz60 .qbtn:hover{background:#f0f8f0;border-color:rgba(30,96,64,.45);color:var(--ink);transform:translateX(3px)}
#quiz60 .qbtn:active{background:#e4f2e4;transform:translateX(3px) scale(.98)}
.qcircle{width:15px;height:15px;border-radius:50%;border:1.5px solid rgba(30,96,64,.3);flex-shrink:0;transition:border-color .16s,background .16s}
#quiz60 .qbtn:hover .qcircle{border-color:#1e6040;background:rgba(30,96,64,.12)}
#quiz60 .qback{margin-top:13px;background:none;border:none;color:rgba(30,60,40,.4);font-size:.8rem;cursor:pointer;padding:0;font-family:var(--sans);transition:color .15s;display:block}
#quiz60 .qback:hover{color:var(--ink)}
...
.qcheck{display:flex;align-items:center;gap:12px;border:1px solid rgba(30,96,64,.18);background:#fff;padding:11px 15px;border-radius:11px;font-size:.88rem;color:var(--ink);cursor:pointer;transition:background .16s,border-color .16s;font-family:var(--sans);line-height:1.4}
.qcheck input[type=checkbox]{display:none}
.qcheck:hover{background:#f0f8f0;border-color:rgba(30,96,64,.4);color:var(--ink)}
.qcheck.qsel{background:#e4f2e4;border-color:#1e6040;color:var(--ink)}
.qcheck.qsel .qcircle{background:#1e6040;border-color:#1e6040}
.qcontinue{margin-top:14px;width:100%;padding:12px 18px;background:var(--sage);border:1px solid var(--sage);color:#fff;border-radius:11px;font-size:.9rem;font-family:var(--sans);cursor:pointer;transition:background .16s,border-color .16s;text-align:center}
.qcontinue:not(:disabled):hover{background:#175233;border-color:#175233}
.qcontinue:disabled{opacity:.35;cursor:not-allowed}
#qmintro .qstartbtn:hover,#qstartbtn:hover{background:#175233!important;border-color:#175233!important}
...
function _qset(vt,nt,bl){
  ...
  document.getElementById('qbullets').innerHTML=bl.map(function(b){return'<div style="display:flex;gap:10px;font-size:.84rem;color:rgba(30,60,40,.85);line-height:1.5"><span style="color:#2d7a4a;flex-shrink:0;margin-top:2px">'+_IC+'</span><span>'+b+'</span></div>';}).join('');
}
...
function _qSetupMulti(optsId,contId){
  document.getElementById(optsId).querySelectorAll('.qcheck').forEach(function(el){
    el.onclick=function(e){e.preventDefault();_qMultiToggle(el,contId);};
  });
}
...
function _qShowResult(){
  ...
  if(cat==='assessment_first'){
    ...
    bl=['Free and completely confidential initial consultation.','We screen for safety concerns before any joint process begins.','Shuttle or online mediation (via Zoom, no direct contact) available where needed.','If mediation isn\\'t appropriate, we\\'ll tell you honestly and point you toward better options.'];
    ...
    if(_qd.type==='property'||_qd.type==='both'){
      bl=['Even difficult negotiations usually settle before reaching court.','Shuttle or online mediation (via Zoom) available if face-to-face isn\\'t workable.','Your solicitor can review any agreement before you sign.','Fixed fee — no billing surprises.'];
    }else if(_qd.type==='parenting'){
      bl=['Shuttle or online mediation (via Zoom) available — no direct contact required.','Safety screening completed before any joint process begins.','Section 60I certificate issued — required for consent order applications.','Child-focused process with full confidentiality protections.'];
    }else{
    ...
    if(_qd.type==='property'){
      ...
      bl=['Completely confidential — nothing leaves the mediation room.','Preserves the employment relationship where possible.','Available in person or online via Zoom — Australia-wide.','Faster and far less costly than Fair Work or tribunal.'];
    }else if(_qd.type==='business'){
      bl=['Commercially sensitive — everything remains private.','Available in person or online via Zoom — Australia-wide.','Binding agreement enforceable as a contract.','A fraction of the cost of commercial litigation.'];
